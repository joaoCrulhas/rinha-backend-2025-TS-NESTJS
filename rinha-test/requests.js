...
export async function setPPToken(service, token) {
  ...
  const response = await httpClient.asyncPut(
    '/admin/configurations/token',
    payload,
    params,
  );
  ...
  if (response.status != 204) {
    exec.test.abort(
      `Erro ao definir token para ${service} (HTTP ${response.status}).`,
    );
  }
}
...
export async function setPPDelay(service, ms) {
  ...
  const response = await httpClient.asyncPut(
    '/admin/configurations/delay',
    payload,
  );

  if (response.status != 200) {
    exec.test.abort(
      `Erro ao definir delay para ${service} (HTTP ${response.status}).`,
    );
  }
}
...
export async function setPPFailure(service, failure) {
  ...
  const response = await httpClient.asyncPut(
    '/admin/configurations/failure',
    payload,
  );

  if (response.status != 200) {
    exec.test.abort(
      `Erro ao definir failure para ${service} (HTTP ${response.status}).`,
    );
  }
}
...
export async function resetPPDatabase(service) {
  ...
  if (response.status != 200) {
    exec.test.abort(
      `Erro ao resetar database para ${service} (HTTP ${response.status}).`,
    );
  }
}
...
export async function getPPPaymentsSummary(service, from, to) {
  const httpClient = paymentProcessorHttp[service];
  const response = await httpClient.asyncGet(
    `/admin/payments-summary?from=${from}&to=${to}`,
  );

  if (response.status != 200) {
    exec.test.abort(
      `Erro ao obter admin payments summary para ${service} (HTTP ${response.status}).`,
    );
  }
...
export async function getBackendPaymentsSummary(from, to) {
  const response = await backendHttp.asyncGet(
    `/payments-summary?from=${from}&to=${to}`,
  );

  if (response.status != 200) {
    exec.test.abort(
      `Erro ao obter payments summary do backend (HTTP ${response.status}).`,
    );
  }
...
export async function requestBackendPayment(payload) {
  const response = await backendHttp.asyncPost(
    '/payments',
